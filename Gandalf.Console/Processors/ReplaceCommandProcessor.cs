...
{
    public class ReplaceCommandProcessor : CommandProcessor, ICommandProcessor
    {
        public ReplaceCommandProcessor(ITelegramBotService service) : base(service)
        {
        }
        ...
        public async Task<bool> Process(string messageText)
        {
            ...
            if (service.Mode == BotMode.File)
            {
                ...
                string newline = messageText.Substring(messageText.IndexOf('\n') + 1).Replace((char)0xa0, ' ');
                var ss2 = ss.Take(line).Concat(new[] { newline }).Concat(ss.Skip(line + 1)).ToArray();
                System.IO.File.WriteAllLines(service.CurrentFile, ss2);
                await service.Bot.SendTextMessageAsync(chatId: service.ChatId, text: "replaced line: " + line, cancellationToken: service.CancellationToken);
            }
